    ...
    case 'near':
      if (operatorValue.maxDistance) {
          return ParameterizedSQL.join([
              new ParameterizedSQL('ST_Distance(' + toPostgisPoint(columnName + '[0]', columnName + '[1]') + ','),
              operatorValue,
...
PostgreSQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type.name === 'GeoPoint' || prop.type.name === 'Point') {
    ...
    // Decorate return value if we need to filter by max distance geometrically.
