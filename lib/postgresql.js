...
PostgreSQL.prototype.buildExpression = function(columnName, operator,
  ...
  switch (operator) {
    ...
    case 'near':
      if (operatorValue.maxDistance) {
          return ParameterizedSQL.join([
              new ParameterizedSQL('ST_Distance(' + toPostgisPoint(columnName + '[1]', columnName + '[0]') + ','),
              new ParameterizedSQL(toPostgisPoint(operatorValue.lat, operatorValue.lng)),//operatorValue,
              new ParameterizedSQL(') < ?', [operatorValue.maxDistance])
          ]);
      } else return;
    default:
...
/**
 * In order to dynamically sort by distance for near clauses we need to hook into the buildSelect
 * and buildOrderBy functions.
 */
PostgreSQL.prototype.buildSelect = function(model, filter, options) {
    // Checking for near clauses is broken in the juggler as it doesn't
    // recursively check for near clauses in and/or statements.
    function getNearClause(where) {
        for (var key of Object.keys(where)) {
            if (key === 'or' || key === 'and') {
                if (Array.isArray(where[key])) {
                    for (var i = 0; i < where[key].length; i++) {
                        var near = getNearClause(where[key][i]);

                        if (near) return near;
                    }
                } else {
                    return getNearClause(where[key]);
                }
            } else {
                if (where[key] && where[key].near) {
                    return {
                        key: key,
                        near: where[key]
                    }
    ...
    // If there is a near clause, we attach a distance ordering to filter.order.
    if (filter.where) {
        var nearClause = getNearClause(filter.where);

        if (nearClause) {
            // The datasource-juggler does not correctly detect near clauses at this time,
            // so this is a hack to circumvent that.
            var {key, near} = nearClause;

            if (this.buildNearFilter) {
                this.buildNearFilter(filter, near);
            } else {
                console.log("WARNING: Using near clause in a database without enableGeoIndexing enabled in loopback settings.");
            }

            var orderClause = {
                type: 'geometric',
                clause: 'ST_Distance('
                    + toPostgisPoint(near.near.lat, near.near.lng) + ','
                    + toPostgisPoint(key + '[1]', key + '[0]') + ')'
            };

            if (filter.order) {
                if (Array.isArray(filter.order)) filter.order.push(orderClause);
                else filter.order = [filter.order, orderClause];
            } else {
                filter.order = [orderClause];
            }
        }
    }

    return this.invokeSuper('buildSelect', model, filter, options);
};

/**
 * We need to roll our own buildOrderBy() function to allow for geometric
 * ordering functions.
 */
PostgreSQL.prototype.buildOrderBy = function(model, order) {
    ...
    var clauses = [];
    for (var clause of order) {
        if (typeof clause === 'string') {
            var t = clause.split(/[\s,]+/);

            if (t.length === 1) {
              clauses.push(this.columnEscaped(model, clause));
            } else {
              clauses.push(this.columnEscaped(model, t[0]) + ' ' + t[1]);
            }
        } else {
            // Handle geometric 'near' clauses.
            if (clause.type === 'geometric') {
...
/**
 * Given a near clause, formats it in a unified manner:
 *   {
 *      near: {
 *          lng: *longitude*,
 *          lat: *latitude*,
 *          maxDistance *max distance to filter by in meters
 *      }
 *   }
 ...
 * See https://loopback.io/doc/en/lb2/Where-filter.html#near-ordering-andlimiting-by-distance
 */
function buildNearFilter(query, near) {
  var coordinates = {};
  if (typeof near.near === 'string') {
    var s = near.near.split(',');
    coordinates.lng = Number(s[1]);
    coordinates.lat = Number(s[0]);
  } else {
    coordinates = near.near;
  }

  near.near = coordinates;

  // Convert maxDistance to meters and attach to near clause if it exists.
  if (near.maxDistance) {
    var unit = near.unit || 'meters';
    near.near.maxDistance = convertToMeters(near.maxDistance, unit);
    near.near.unit = 'meters';
    delete near.maxDistance;
    delete near.unit;
  }
};

// Converts a distance to meters, for use with near clauses in where filters.
// See https://loopback.io/doc/en/lb2/Where-filter.html#near-ordering-andlimiting-by-distance
function convertToMeters(distance, unit) {
  switch (unit) {
    ...
    default:
      console.warn('Unsupported unit ' + unit + ', falling back to meters as default.');
      return distance;
  }
}

// Given a latitude and longitude, produce SQL for a WGS84 PostGIS geography point.
function toPostgisPoint(latitude, longitude) {
    return "ST_SetSRID(ST_Point(" + longitude + ", " + latitude + "), 4326)::geography";
}
...
PostgreSQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type.name === 'GeoPoint' || prop.type.name === 'Point') {
    var geopoint = new ParameterizedSQL({
      sql: 'point(?,?)',
      // Postgres point is point(lng, lat)
      params: [val.lng, val.lat],
    });

    // Decorate result with geometric data for use in near filters.
    if (val.maxDistance) geopoint.maxDistance = val.maxDistance;
    geopoint.lng = val.lng;
    geopoint.lat = val.lat;
