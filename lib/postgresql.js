...
PostgreSQL.prototype.buildExpression = function(columnName, operator,
  ...
  switch (operator) {
    ...
    case 'near':
      ...
      console.log("Constructing maxDistance thing for geometric point.");
...
PostgreSQL.prototype.buildSelect = function(model, filter, options) {
    ...
    if (filter.where) {
        var near = getNearClause(filter.where);

        // This assumption isn't true right now (as juggler near-clause detection
        // is broken), but the near clause should have been properly formatted by
        // nearFilter().
        if (near) {
            var orderClause = {
                type: 'geometric',
                clause: 'ST_Distance(ST_Point(' + near.near.lng + ',' + near.near.lat + '), '
                    + 'ST_Point(' + near.key + '[0],' + near.key + '[1]))'
            };
